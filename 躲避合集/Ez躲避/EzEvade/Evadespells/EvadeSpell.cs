...
namespace ezEvade
{
    class EvadeSpell
    {
        ...
        public EvadeSpell(Menu mainMenu)
        {
            ...
            evadeSpellMenu = mainMenu.AddSubMenuEx("躲避技能", "EvadeSpells");
        ...
        private static Menu CreateEvadeSpellMenu(EvadeSpellData spell)
        {
            ...
            evadeSpellMenu.Add(spell.name + "UseEvadeSpell", new CheckBox("使用技能", true));
            evadeSpellMenu.Add(spell.name + "EvadeSpellDangerLevel", new Slider("危险等级", spell.dangerlevel = 1, 0, 3));
            var slider = evadeSpellMenu.Add(spell.name + "EvadeSpellMode", new Slider("使用技能模式", GetDefaultSpellMode(spell), 0, 2));
            var array = new[] { "不可躲避的", "激活时间", "总是" };
