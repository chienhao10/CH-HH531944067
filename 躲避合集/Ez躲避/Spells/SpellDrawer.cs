...
namespace ezEvade
{
    internal class SpellDrawer
    {
        ...
        private void Game_OnGameLoad()
        {
            ...
            Menu drawMenu = menu.IsSubMenu ? menu.Parent.AddSubMenuEx("线圈", "Draw") : menu.AddSubMenuEx("显示", "Draw");
        ...
        private void DrawEvadeStatus()
        {
            if (ObjectCache.menuCache.cache["ShowStatus"].Cast<CheckBox>().CurrentValue)
            {
                ...
                if (ObjectCache.menuCache.cache["DodgeSkillShots"].Cast<KeyBind>().CurrentValue)
                {
                    if (Evade.isDodging)
                    {
                        Drawing.DrawText(heroPos.X - dimension.Width / 2, heroPos.Y, Color.Red, "躲避: 开");
                    }
                    else
                    {
