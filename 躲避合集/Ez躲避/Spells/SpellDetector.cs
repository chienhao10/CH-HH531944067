...
namespace ezEvade
{
    ...
    internal class SpellDetector
    {
        ...
        public SpellDetector(Menu mainMenu)
        {
            ...
            spellMenu = mainMenu;
            spellMenu = mainMenu.AddSubMenuEx("技能", "Spells");
        ...
        public static void LoadDummySpell(SpellData spell)
        {
            ...
            spellMenu.Add(spell.spellName + "DodgeSpell", new CheckBox("躲避技能", enableSpell));
            spellMenu.Add(spell.spellName + "DrawSpell", new CheckBox("显示技能", enableSpell));
            spellMenu.Add(spell.spellName + "SpellRadius",new Slider("显示半径", (int) spell.radius, (int) spell.radius - 100, (int) spell.radius + 100));
            spellMenu.Add(spell.spellName + "FastEvade", new CheckBox("强制快速躲避", spell.dangerlevel == 4));
            spellMenu.Add(spell.spellName + "DodgeIgnoreHP",new Slider("无视当血量 %", spell.dangerlevel == 1 ? 80 : 100));
            var slider = spellMenu.Add(spell.spellName + "危险等级",
                new Slider("危险等级", spell.dangerlevel - 1, 0, 3));
            var array = new[] {"低", "正常", "高", "极高"};
