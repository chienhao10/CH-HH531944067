...
namespace DrVayne.Common
{
    public class TargetSelector2
    {
        ...
        internal static void Initialize()
        {
            ...
            {
                config = MainMenu.AddMenu("优先攻击目标(W)", "TargetSelector2");
                config.AddLabel("目标选择模式解释（按顺序）：0自动，1低血量，2最高AD,3最高AP");
                config.AddLabel("4离薇恩最近的，5距离鼠标最近的，6最少攻击可击杀的，7最少技能可击杀的");

                _configMenu = config;

                config.Add("FocusSelected", new CheckBox("优先攻击鼠标点击的目标", true));
                config.Add
                    ("ForceFocusSelected", new CheckBox("只攻击鼠标点击的目标", false));
                config.Add
                    ("DrawTarget", new CheckBox("选择的目标显示圆圈", false));
                config.AddSeparator(10);
                var autoPriorityItem =
                    config.Add("AutoPriority", new CheckBox("自动调整攻击顺序", true));
                ...
                config.AddStringList("TargetingMode", "目标选择模式", (Enum.GetNames(typeof(TargetingMode))), 0);
        ...
        {
            config.AddSubMenu("警报", "----在Target Selector里设置----");
        }
